...
class STL10(CIFAR10):
    ...
    def __init__(self, root, split='train', show=False, transform=None, transform_aug=None,
                 download=False, num_trans_aug=1):
    ...
    def __getitem__(self, index):
        ...
        img_trans = []
        for i in range(self.num_trans_aug):
            if self.transform_aug is not None:
                img_trans.append(self.transform_aug(img))

        if self.show:
            img_mean = np.array([114, 112, 104]) / 255.
            if self.transform is not None:

                img_ori_i = img_ori.numpy().transpose([1, 2, 0]) + img_mean
                plt.figure()
                plt.imshow(img_ori_i)

            for i in range(self.num_trans_aug):
                img_trans_i = img_trans[i]
                img_trans_n = img_trans_i.numpy().transpose([1, 2, 0]) + img_mean
                plt.figure()
                plt.imshow(img_trans_n)
            plt.show()

        if len(img_trans) == 0:
            return img_ori, -1, target, index
        elif len(img_trans) == 1:
            return img_ori, img_trans[0], target, index
        else:
            return img_ori, img_trans, target, index
